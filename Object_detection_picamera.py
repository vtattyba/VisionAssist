...
#Initialize Coords 
TL_middle = (int(IM_WIDTH*0.5),int(IM_HEIGHT))
BR_middle = (int(IM_WIDTH*0.5),int(IM_HEIGHT*0))

TL_left = (int(IM_WIDTH*0),int(IM_HEIGHT))    
BR_left = (int(IM_WIDTH*0.35),int(IM_HEIGHT*0))


TL_right = (int(IM_WIDTH*0.65),int(IM_HEIGHT))
...
for frame1 in camera.capture_continuous(rawCapture, format="bgr",use_video_port=True):
    ...
    if (int(classes[0][0] == 1)):
        ...
        """
        if ((x > BR_left[0]) and (x < TL_middle[0])):
            cv2.putText(frame,'Go Right',(30,100),font,1,(20,255,255),2,cv2.LINE_AA)  
            GPIO.setup(12,GPIO.OUT)
            GPIO.output(12,0)
            time.sleep(.3)
            GPIO.output(12,1)
        ...
        if ((x < TL_right[0]) and (x > TL_middle[0])):
            cv2.putText(frame,'Go Left',(30,100),font,1,(20,255,255),2,cv2.LINE_AA)   
            GPIO.setup(12,GPIO.OUT)
            GPIO.output(12,0)
            time.sleep(.3)
            GPIO.output(12,1)
